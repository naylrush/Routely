//
// Copyright © 2025 Движ
//

extension RouterState {
    public convenience init<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>(
        _ state: RouterState<CompositeRoute<TargetRoute>>
    ) where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        let path = state.path.compactMap(IdentifiableModel<Route>.init)
        let presentationState = state.presentationState.flatMap(PresentationState<Route>.init)
        self.init(path: path, presentationState: presentationState)
    }

    public func into<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>()
    -> RouterState<CompositeRoute<TargetRoute>>
    where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        .init(
            path: path.compactMap { $0.into() },
            presentationState: presentationState?.into()
        )
    }
}

extension PresentationState {
    public init?<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>(
        _ state: PresentationState<CompositeRoute<TargetRoute>>
    ) where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        guard let routeWithResult = RouteWithResult<Route>(state.routeWithResult) else { return nil }
        self.init(style: state.style, routeWithResult: routeWithResult)
    }

    public func into<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>()
    -> PresentationState<CompositeRoute<TargetRoute>>?
    where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        guard let target: RouteWithResult<CompositeRoute<TargetRoute>> = routeWithResult.into() else { return nil }
        return .init(style: style, routeWithResult: target)
    }
}

extension RouteWithResult {
    public init?<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>(
        _ value: RouteWithResult<CompositeRoute<TargetRoute>>
    ) where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        guard let route = Route(value.route) else { return nil }
        self.init(route: route, result: value.result)
    }

    public func into<WrappedRoute: ConvertibleRoutable, TargetRoute: Routable>()
    -> RouteWithResult<CompositeRoute<TargetRoute>>?
    where Route == CompositeRoute<WrappedRoute>, WrappedRoute.Target == TargetRoute {
        guard let target: CompositeRoute<TargetRoute> = route.into() else { return nil }
        return .init(route: target, result: result)
    }
}

extension IdentifiableModel {
    public init?<Route: ConvertibleRoutable, TargetRoute: Routable>(
        _ model: IdentifiableModel<CompositeRoute<TargetRoute>>
    ) where T == CompositeRoute<Route>, Route.Target == TargetRoute {
        guard let value = T(model.value) else { return nil }
        self.init(id: model.id, value: value)
    }

    public func into<Route: ConvertibleRoutable, TargetRoute: Routable>()
    -> IdentifiableModel<CompositeRoute<TargetRoute>>?
    where T == CompositeRoute<Route>, Route.Target == TargetRoute {
        guard let target: CompositeRoute<TargetRoute> = value.into() else { return nil }
        return .init(id: id, value: target)
    }
}

extension CompositeRoute where Wrapped: ConvertibleRoutable {
    public init?<NestedWrapped: Routable>(
        _ route: CompositeRoute<NestedWrapped>
    ) where Wrapped.Target == NestedWrapped {
        if case let .wrapped(nestedWrapped) = route, let base = Wrapped(nestedWrapped) {
            self = .wrapped(base)
        } else {
            return nil
        }
    }

    public func into<NestedWrapped: Routable>()
    -> CompositeRoute<NestedWrapped>?
    where Wrapped.Target == NestedWrapped {
        if case let .wrapped(wrapped) = self, let nestedWrapped = wrapped.into() {
            .wrapped(nestedWrapped)
        } else {
            nil
        }
    }
}
